                        ...
                        for element in element_group.get("data", []):
                            if (element["end_ms"] < next_step_ms and element["end_ms"] >= current_step_ms):
                                step_events.append({
                                    ...
                                    "element": element_to_vector.get(element["element"], "100000000")
                                })
                            elif ((element["start_ms"] >= current_step_ms and element["start_ms"] < next_step_ms) or (element["start_ms"] < current_step_ms and element["end_ms"] > next_step_ms)):
                                step_events.append({
                                    "step_ms": current_step_ms,
                                    "frequency": normalize_frequency(element_group["frequency"]),
                                    "speed_wpm": normalize_speed_wpm(element_group["speed_wpm"]),
                                    "element": "010000000"  # element_to_vector.get(element["element"], "100000000")
                                })

                    # Dodanie elementów zerowych jeśli jest mniej niż 5 elementów
                    while len(step_events) < 10:
                        step_events.append({
                            "step_ms": current_step_ms,
                            "frequency": 0,
                            "speed_wpm": 0,
