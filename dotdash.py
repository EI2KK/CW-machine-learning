...
def generate_morse_data(char, start_time, speed, max_end_time):
    ...
    for index, element in enumerate(morse_elements):
        ...
        if next_time > max_end_time:
            break
        data.append({"element": "dot" if element == '.' else "dash", "start_ms": start_time, "end_ms": next_time,
                     "duration_ms": next_time - start_time})
        ...
        data.append({"element": "element_end", "start_ms": start_time, "end_ms": start_time + unit_duration,
                     "duration_ms": unit_duration})
        start_time = element_gap

    data.append({"element": "char_end", "start_ms": start_time, "end_ms": start_time + (3 * unit_duration),
                 "duration_ms": (3 * unit_duration)})
...
def generate_word_data(word, start_time, speed, max_end_time):
    ...
    if word_end_time > (total_length - stop_b4_end):
        data.append({"element": "word_end", "start_ms": word_start_time, "end_ms": word_end_time,
                     "duration_ms": word_end_time - word_start_time})
        data.append({"element": "pause", "start_ms": word_start_time, "end_ms": word_start_time + pause_length,
                     "duration_ms": pause_length})
    ...
    if word_end_time <= max_end_time:
        ...
        if (max_end_time - word_start_time) > pause_length:
            ...
            if x <= pause_probablilty:
                ...
                data.append({"element": "pause", "start_ms": word_start_time, "end_ms": word_end_time,
                             "duration_ms": pause_length})
...
def generate_frequencies(nr_of_freq, freq_range, min_sep, sidetone=base_frequency):
    ...
    # Usuwamy częstotliwości zbyt blisko sidetone
    for freq in range(sidetone - min_sep, sidetone + min_sep + 1):
    ...
    while len(frequencies) < nr_of_freq and possible_frequencies:
        ...
        # Usuwamy częstotliwości zbyt blisko wybranej oraz sidetone
...
for file_number in range(1, num_files_to_generate + 1):
    ...
    for _ in range(nr_of_freq):
        ...
        start_time = random.randint(*start_between_ms)
        volume = round(random.uniform(min_volume, 1), 3)
        freq_data = {"frequency": frequency, "speed_wpm": speed, "output": 0, "volume": volume,
                     "data": []}  # Dodano 'volume'
    ...
    for _ in range(nr_of_qrm_freq):
        ...
        volume = round(random.uniform(min_volume, 1), 3)
        qrm_data = {"frequency": frequency, "speed_wpm": 0, "output": 0, "volume": volume,
                    "data": []}  # Dodano 'volume'
        ...
        data = []
        data.append({"element": "qrm", "start_ms": start_time, "end_ms": start_time + qrm_duration,
                     "duration_ms": qrm_duration})
