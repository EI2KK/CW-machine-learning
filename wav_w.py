...
def generate_cw_signal(words, speed_wpm, frequency, start_times_ms, file_duration_ms, amplitude=0.5, sample_rate=44100, inaccuracy=0, frequency_inaccuracy=0):
    total_length = int(sample_rate * file_duration_ms / 1000)
    main_signal = np.zeros(total_length)

    for word, start_time_ms in zip(words, start_times_ms):
        ...
        for character in word:
            ...
            word_signal = np.concatenate([word_signal, char_signal])
            inter_char_silence = np.zeros(dot_length * 2)
            word_signal = np.concatenate([word_signal, inter_char_silence])
        ...
        end_sample = min(start_sample + len(word_signal), total_length)
        main_signal[start_sample:end_sample] += word_signal[:end_sample - start_sample]
